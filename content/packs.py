...
def open_gift4(uid, cdata, edit, cid):
    ...
    if cdata.startswith('gift_unpack_2026_'):
        ...
        if ran == 1:
            ...
            if effect == 1:
                r.hincrby(uid, 'strength', 1)
                msg = '⚪ У Донбаському рюкзаку лежить шматочок ковбаси.\n💪 +1'
            elif effect == 2:
                r.hincrby(uid, 'injure', 1)
                msg = '⚪ На дні Донбаського рюкзака лежав циркуль.\n🩸 +1'
            elif effect == 3:
                r.hincrby(uid, 'sch', 1)
                msg = '⚪ Русак переплутав Донбаські рюкзаки із сусідом, на що той вдарив його по голові.\n👤 +1'
            elif effect == 4:
                increase_trance(1, uid)
                msg = '⚪ У Донбаському рюкзаку лежить кіндер-сюрприз.\n👊 +1'
            else:
                hp(1, uid)
            ...
            msg = '🔵 Хтось поклав у цей Донбаський рюкзак цукерки Рошен.\n🧪 +2'
            if int(r.hget(uid, 'support')) == 0:
            ...
            msg = f'🟣 У цьому Донбаському рюкзаку лежить упаковка Артемсолі.\n🧂 +{salt}'
        elif ran == 9:
            r.hincrby(uid, 'money', 500)
            msg = '🟣 У Донбаському рюкзаку лежить конверт на штори.\n💵 +500'
        elif ran == 10:
            casino = choice(['🎯', '🎲', '🎳', '⚽', '🏀', '🎰'])
            r.hincrby(casino, uid, 5)
            msg = f'🟣 У Донбаському рюкзаку лежать старі жетони для ігрових автоматів...\n'
            msg += f'Русак може випробувати свою удачу.\n{casino} +5\n/casino'
        elif ran == 11:
            r.hincrby(uid, 'tape', 1)
            msg = '🟣 У Донбаському рюкзаку лежить якийсь чорний моток.\n🌀 +1'
        elif ran == 12:
            try:
            ...
            r.hincrby(uid, 'strap', 1)
            msg = '🟡 У Донбаському рюкзаку лежить стара армійська форма.\n🌟 +1'
        else:
            r.sadd('prigozhin_2026', uid)
